...
public class IntersectionBenchmarks {
    ...
    private final Map<String, Polygon.SimplePolygon[]> geographicData = new LinkedHashMap<>();
    private final Map<String, Polygon.SimplePolygon[]> cartesianData = new LinkedHashMap<>();
    private final Intersect geographicNaiveCalculator = IntersectCalculator.getCalculator(CRS.WGS84, IntersectCalculator.AlgorithmVariant.NAIVE);
    private final Intersect geographicSweepCalculator = IntersectCalculator.getCalculator(CRS.WGS84, IntersectCalculator.AlgorithmVariant.MC_SWEEP_LINE);
    private final Intersect cartesianNaiveCalculator = IntersectCalculator.getCalculator(CRS.CARTESIAN, IntersectCalculator.AlgorithmVariant.NAIVE);
    private final Intersect cartesianSweepCalculator = IntersectCalculator.getCalculator(CRS.CARTESIAN, IntersectCalculator.AlgorithmVariant.MC_SWEEP_LINE);
    ...
    public void testCartesianIntersectPolygonsNaive(Blackhole bh) {
        cartesianData.forEach((region, polygons) -> {
    ...
    public void testGeographicIntersectPolygonsNaive(Blackhole bh) {
        geographicData.forEach((region, polygons) -> {
            for (int i = 0; i < polygons.length; i++) {
                for (int j = i + 1; j < polygons.length; j++) {
